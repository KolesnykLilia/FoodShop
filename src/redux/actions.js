...
export function addProducts(){
	return async dispatch => {
		try{
			...
			const docRef = doc(db, "products", 'D9Boca4VPzQvBR3jH02e');
			const colRef = collection(docRef, "contentsProducts")	
			const newProduct = await addDoc(colRef, {	
				imgUrl: 'gs://shop-6f3b6.appspot.com/hygiene/775741_480x480wwm_bfdcd9f5-411d-61c5-d27b-3d1116d7c73e.png ',
				mark: 0, // don't touch
				marks: [], // don't touch
				info: [
					{ 
				...
				name: 'Щітка зубна Upwill в асортименті',
				price: 102.00,
				oldPrice: 0.00,
				type: 'шт',
				weight: 1,
				typeCategory: 'Догляд за ротовою порожниною',
				id: '' // don't touch
			})
